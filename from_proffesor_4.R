...
#Before deleting the outlier model linearity will be examined
crPlots(unmodified_model)

#remove the record from training dataset
training_data <- subset(training_data, training_data$name != "Nevada")

#Rebuild the model using 
unmodified_model <- lm(Murder ~ 
                         Population + 
                         Income + 
                         Illiteracy + 
                         Life_Exp + 
...
#remove Maine record from training dataset
...
#where n = sample size, and k is number of predictor variables
...
#homoscedacity = the data needs to have a constant variance in the residuals
#If the p-value < 0.05 then error variance changes with level of fitted values
#means need to remove outliers
ncvTest(unmodified_model)

#Model transform?
spreadLevelPlot(unmodified_model)

#Suggested power transformation:  0.7360597 
#so need to do square root  (refer lamda table in ppt)
sqrt_murder <- sqrt(training_data$Murder)

unmodified_model_sqrt <- lm(sqrt_murder ~ 
                         Population + 
                         Income + 
                         Illiteracy + 
                         Life_Exp + 
...
#for comparison
summary(unmodified_model)
#earlier model has better r2 value (high) so it is good compared to sqrt transformation
...
actual_predictions <- data.frame(cbind(actuals = testing_data$Murder, 
...
summary(states)
#What is the murder rate where the population =1000,
#income = 4000, illiteracy = 5%
#life expectancy is 70, and high school grade = 37%
...
#How could we check if 4.99% is an accurate reflection of the real data
