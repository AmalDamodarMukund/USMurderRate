...
# Need to transform illiteracy, HS_grad and population
#Find optimal values of lambda to transform the data

install.packages("MASS")
library(MASS)
#plot the lambda from -6 to 6 in steps of 0.1
Box <- boxcox(states$Population ~ 1,
              lambda = seq(-6, 6, 0.1))

#Create a dataframe containing the results
Cox <- data.frame(Box$x, Box$y)

#Order the new dataframe by decreasing y
...
transformed_population <- (Population ^ lambda - 1)/ lambda
...
lambda
attach(states)
transformed_HS_Grad <- (HS_Grad ^ lambda - 1)/ lambda
...
# Transform Illiteracy

#plot the lambda from -6 to 6 in steps of 0.1
Box <- boxcox(states$Illiteracy ~ 1,
              lambda = seq(-6, 6, 0.1))
...
lambda
attach(states)
transformed_Illiteracy <- (Illiteracy ^ lambda - 1)/ lambda
...
# p-value = 0.004267 - Illiteracy is still not normally distributed

# Insert transformed variables into the states data frame
...
#Build 2 models - 1 = not transformed data, 2 = transformed data

#not transformed data
unmodified_model <- lm(Murder ~ Population +
                         Illiteracy +
                         Life_Exp +
                         HS_Grad + 
                         Frost, 
...
#Evaluate the models
#not transformed model evaluation
install.packages("leaps")
library(leaps)
MLR_subset_selection <- regsubsets(Murder ~ Population +
                                     Illiteracy +
                                     Life_Exp +
                                     HS_Grad + 
                                     Frost, 
...
# adjr2 means adjusted r^2 value
...
#Smallest AIC model is best
stepAIC(unmodified_model, direction = "backward")

stepAIC(modified_model, direction = "backward")

#unmodified model vs modified model? - Not much difference

#which variables to keep in the model?
#Using population, illiteracy, life_exp, hs_grad
#Drop frost - because believing that there is no correlation, only causation

murder_model <- lm(Murder ~ Population + 
                     Illiteracy +
                     Life_Exp +
                     HS_Grad,
                   data = training_data)
...
qqPlot(murder_model, 
       ...
       id.method = "identify",
       simulate = TRUE,
       main = "Q-Q plot for murder model")


training_data["Alabama",]

fitted(murder_model)["Alabama"]
